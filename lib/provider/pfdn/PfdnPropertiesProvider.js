...
PfdnPropertiesProvider.prototype.updateDrawing = function(definition){
  if (is(definition, 'pfdn:Settings')){
    ...
    this._modelling._elements.label._labels._builder(definition.id, definition);
  } else if (is(definition, 'pfdn:Link')){
    this._modelling._elements.link._links.appendElement(definition.id, definition);
    this._modelling._elements.label._labels.appendElement(definition.label.id, definition.label);
  } else if (is(definition, 'pfdn:Node')){
    this._modelling._elements.node._nodes.appendElement(definition.id, definition);
    this._modelling._elements.label._labels.appendElement(definition.label.id, definition.label);
  }
};
